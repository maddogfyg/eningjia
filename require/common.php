...
function numofpage($count,$page,$numofpage,$url,$max=null,$ajaxurl='') {
	...
	if ($numofpage <= 1 || !is_numeric($page)) {
		...
		$mao && $mao = '#'.$mao;
		$pages = "<div class=\"pages cc\"><a href=\"{$url}page=1$mao\" class=\"b\"".($ajaxurl ? " onclick=\"return ajaxview('{$ajaxurl}page=1')\"" : '').">&laquo;</a>";
		...
		$pages .= "<a href=\"{$url}page=$numofpage$mao\" class=\"b\"".($ajaxurl ? " onclick=\"return ajaxview('{$ajaxurl}page=$numofpage')\"" : '').">&raquo;</a><span class=\"pagesone\">Pages: $page/$total&nbsp; &nbsp; &nbsp;Go <input type=\"text\" size=\"3\" onkeydown=\"javascript: if(event.keyCode==13){".($ajaxurl ? "ajaxview('{$ajaxurl}page='+this.value)" : " location='{$url}page='+this.value+'{$mao}';")."return false;}\"></span></div><div class=\"c\"></div>";
		return $pages;
	}
}
/**
...
class L {
	...
	function loadClass($className,$dir ='',$check=false) {
		...
		if ($dir) {
			...
			if ($dir == 'db') {
				...
				$fileDir = Pcv(R_P.'lib/'.$dir.'/'.strtolower($className).$suffix);
			}
		} else {
			$fileDir = Pcv(R_P.'lib/'.strtolower($className).$suffix);
		}
		...
		if (file_exists($fileDir)) {
			$class = 'PW_'.$className;
			if (!class_exists($class)) require $fileDir;
	...
	function style($var = null, $skinco = null) {
		...
		return L::config($var, $skin, 'style');
	}
