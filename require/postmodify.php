...
$t_per = 0;$t_exits = 0;$t_sub_exits = 0;
...
$db_forcetype = $t_exits && $t_per=='2' && $article==0 && !$pwpost->admincheck ? 1 : 0; // 是否需要强制主题分类
...
if ($postmodify->type == 'topic') {
	...
	$pcid = $atcdb['special'] - 20;
} else {
	$special = $cateid = $modelid = $pcid = 0;
}
if ($cateid && $modelid) {//分类主题
...
if (empty($_POST['step'])) {
	...
	refreshto("read.php?tid=$tid&page=$page&toread=1#$pid", $postdata->getIfcheck() ? 'enter_thread' : 'post_check');
}
?>