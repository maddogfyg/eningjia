...
function updatecache_forums($fids) {
	...
	if (!empty($fids)) {
		$query	= $db->query("SELECT f.*,fe.creditset,fe.forumset,fe.commend FROM pw_forums f LEFT JOIN pw_forumsextra fe ON f.fid=fe.fid WHERE f.fid IN (".pwImplode($fids,false).")");
		while ($forums = $db->fetch_array($query)) {
			writeforumscache($forums);
		}
	}
...
function writeforumscache($forum) {
	...
	foreach ($forum as $key => $value) {
		if ($value && in_array($key,array('creditset','forumset','commend'))){
...
function updatecache_ad() {
	...
	while($rt = $db->fetch_array($query)){
		...
		if ($conf['type'] == 'code') {
			...
			$conf['width'] && $style .= " width=\"$conf[width]\"";
			$conf['height'] && $style .= " height=\"$conf[height]\"";
			$code = "<embed src=\"$conf[link]\" type=\"application/x-shockwave-flash\"{$style}></embed>";
		}
		if ($conf['winHeight']) {
...
function updatecache_w() {
	...
	$query = $db->query("SELECT * FROM pw_wordfb ORDER BY id");
	while (@extract($db->fetch_array($query))) {
		if ($word) {
...
function updatecache_plan() {
	global $db;
	$nexttime = $db->get_value("SELECT MIN(nexttime) FROM pw_plan WHERE ifopen='1'");
...
function setConfig($key, $value, $decrip = null) {
	...
	$db->pw_update(
		...
		"UPDATE pw_config SET " . pwSqlSingle($pwSQL) . ' WHERE db_name=' . pwEscape($key),
		"INSERT INTO pw_config SET " . pwSqlSingle(array_merge(array('db_name' => $key), $pwSQL))
	);
}
?>