...
if (empty($action)) {
	...
	$rt = $db->get_one("SELECT fid FROM pw_forums WHERE fid=".pwEscape($fid)." AND type!='category'");
	if (!$rt) {
	...
	if (empty($_POST['step'])) {
		...
		InitGP(array('creditdb','otherfid','otherforum'),'P');
		...
		if (is_array($otherfid) && is_array($otherforum)) {
			foreach ($otherfid as $key => $selfid) {
				...
				$rt = $db->get_one("SELECT fid,creditset FROM pw_forumsextra WHERE fid=".pwEscape($selfid));
				if ($rt['fid']) {
					if ($rt['creditset']) {
						...
						$db->update("UPDATE pw_forumsextra SET creditset=".pwEscape($newcreditset,false).'WHERE fid='.pwEscape($selfid));
					} else {
						$newcreditset = array();
						foreach ($creditdb as $key => $value) {
							foreach ($value as $k => $val) {
					...
					$newcreditset = serialize($newcreditset);
					$db->update("INSERT INTO pw_forumsextra SET creditset=".pwEscape($newcreditset,false).',fid='.pwEscape($selfid));
				}
			}
		}
		$basename = "$admin_file?adminjob=setforum&action=credit&fid=$fid";
	...
	if (empty($_POST['step'])) {
		...
		if ($type <> 'category') {
			...
			${'forumcate_'.$forumcate} = 'checked';
			if ($forumcate == 1) {
				$style2 = $style3 = 'style="display:none"';
			} elseif ($forumcate == 2) {
				$style1 = $style3 = 'style="display:none"';
			} else {
				$style1 = $style2 = 'style="display:none"';
			}
		...
		if ($forum['type'] == 'category') {
			...
			InitGP(array('forumsetdb','uploadset','rewarddb','cfup','ffup','showsub','ifhide', 'viewsub','allowhide','allowsell','copyctrl','f_check','password','allowvisit','allowread', 'allowpost','allowrp','allowupload','allowdownload','otherfid','otherforum','allowtime','allowtype', 'recycle','forumsell','sdate','cprice','rprice','logotype','logo_upload','logo_url','ifdellogo','t_view_db','new_t_view_db','new_t_sub_view_db','t_type','forumcate','cateid','modelid','pcid'),'P');
			...
			foreach ($forumsetdb as $key => $value) {
				...
				} elseif (!in_array($key,array('orderway','asc','commendlist','chat','relatedcon','relatedcustom'))) {
			...
			if ($forumcate == 1 || $forumcate == 2) {
				$pcids = '';
			} elseif ($forumcate == 1 || $forumcate == 3) {
				$modelids = '';
			}
			...
			$db->update("UPDATE pw_forums SET " . pwSqlSingle(array(
				...
				'forumcate'	=> $forumcate,		'cateid'	=> $cateid,
		...
		if ($otherfids && $update_f) {
			...
			foreach ($otherfid as $key => $selfid) {
				...
				if ($rt['fid']) {
					...
					$db->update("UPDATE pw_forumsextra SET forumset=".pwEscape($forumset,false)."WHERE fid=".pwEscape($selfid));
				} else {
					$newforumset = array();
					foreach ($forumsetdb as $key => $value) {
						if ($otherforum['forumsetdb'][$key]) {
