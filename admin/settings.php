...
$basedb = array('basic','safe','att','credit','reg','member','pcache','index','thread','read','jsinvoke','popinfo','wap','email');
$settingdb = array();
...
if ($_POST['step'] != 2) {
	...
	if ($admintype=='basic' || $settingdb['basic']) {
		...
		foreach ($ltitle as $key => $value) {
			if ($key!=1 && $key!=2) {
				...
				$visitgroup .= '<td><input type="checkbox" name="visitgroup[]" value="'.$key.'" '.$s_checked.'>'.$value.'</td>'.$htm_tr;
			}
		}
		$visitgroup && $visitgroup = '<table cellspacing="0" cellpadding="0" border="0" width="100%" align="center"><tr>'.$visitgroup.'</tr></table>';
		//global
		$sltstyles = '';
	...
	if ($admintype=='member' || $settingdb['member']) {
		...
		foreach ($credit->cType as $key => $value) {
			...
			$sellset .= ' <input type="checkbox" name="sellset[type][]" value="'.$key.'" '.$sellcked.' /> '.$value;
			$enhidechked  = in_array($key,$db_enhideset['type']) ? 'CHECKED' : '';
			$enhideset .= ' <input type="checkbox" name="enhideset[type][]" value="'.$key.'" '.$enhidechked.' /> '.$value;
			//face
			if (!is_numeric($key)) {
		...
		foreach ($ltitle as $key => $value) {
			if ($key!=1 && $key!=2) {
				...
				$signgroup .= '<td><input type="checkbox" name="signgroup[]" value="'.$key.'" '.$signcked.'> '.$value.'</td>'.$htm_tr;
			}
		}
	...
	if ($admintype=='read' || $settingdb['read']) {
		...
		foreach ($_CREDITDB as $key => $value) {
			...
			$ctcked = strpos($db_showcustom,",$key,")!==false ? 'CHECKED' : '';
			$showcustom .= '<td><input type="checkbox" name="showcustom['.$key.']" value="'.$key.'" '.$ctcked.'> '.$value[0].'</td>'.$htm_tr;
		}
		$showcustom && $showcustom = '<table cellspacing="0" cellpadding="0" border="0" width="100%" align="center"><tr>'.$showcustom.'</tr></table>';
	...
	if ($admintype == 'safe' || $settingdb['safe']) {
		...
		//seo
		$bbstitle['index'] = strip_tags($bbstitle['index']);
		$bbstitle['other'] = strip_tags($bbstitle['other']);
		$bbstitle = Char_cv($bbstitle,false,true);
		$config['bbstitle'] = is_array($bbstitle) ? $bbstitle : array();
		$config['metakeyword'] = Char_cv(strip_tags($config['metakeyword']));
		$config['metadescrip'] = Char_cv(strip_tags($config['metadescrip']));
		//gdcode
		$config['gdsize']  = implode("\t",$gdsize);
		$config['safegroup'] = $safegroup ? ','.implode(',',$safegroup).',' : '';
		$config['qcheck'] = $qcheck['regq']."\t".$qcheck['loginq']."\t".$qcheck['postq']."\t".$qcheck['msgq'];
		...
		if (!$config['picpath']) {
			if (is_dir('images')) {
				$config['picpath'] = 'images';
			} else {
				adminmsg('settings_picdir_dfnotfind');
			}
		} elseif (!is_dir(R_P.$config['picpath']) && $db_picpath!=$config['picpath']) {
			!preg_match('/^[a-zA-Z0-9\_]+$/i',$config['picpath']) && adminmsg('settings_picdir_error');
			if (!$db_picpath || !is_dir(R_P.$db_picpath)) {
				if (is_dir('images')) {
					$db_picpath = 'images';
				} else {
					adminmsg('settings_picdir_dfnotfind');
			...
			if (!rename(R_P.$db_picpath,R_P.$config['picpath'])) {
				$config['picpath'] = $db_picpath;
			}
		}
		if (!$config['attachname']) {
			if (is_dir('attachment')) {
				$config['attachname'] = 'attachment';
			} else {
				adminmsg('settings_attdir_dfnotfind');
			}
		} elseif (!is_dir(R_P.$config['attachname']) && $db_attachname!=$config['attachname']) {
			!preg_match('/^[a-zA-Z0-9\_]+$/i',$config['attachname']) && adminmsg('settings_attdir_error');
			if (!$db_attachname || !is_dir(R_P.$db_attachname)) {
				if (is_dir('attachment')) {
			...
			if (!rename(R_P.$db_attachname,R_P.$config['attachname'])) {
				$config['attachname'] = $db_attachname;
			}
		}
		if (!$config['htmdir']) {
			if (is_dir('htm_data')) {
				$config['htmdir'] = 'htm_data';
			} else {
				adminmsg('settings_htmdir_dfnotfind');
			}
		} elseif (!is_dir(R_P.$config['htmdir']) && $db_htmdir!=$config['htmdir']) {
			!preg_match('/^[a-zA-Z0-9\_]+$/i',$config['htmdir']) && adminmsg('settings_htmdir_error');
			if (!$db_htmdir || !is_dir(R_P.$db_htmdir)) {
				if (is_dir('htm_data')) {
					$db_htmdir = 'htm_data';
				} else {
					adminmsg('settings_htmdir_dfnotfind');
			...
			if (!rename(R_P.$db_htmdir,R_P.$config['htmdir'])) {
				$config['htmdir'] = $db_htmdir;
			}
		}
		if ($config['http']!='N') {
			if (!$config['http']) {
	...
	if ($admintype == 'credit' || $settingdb['credit']) {
		InitGP(array('creditpay','creditset'),'P');
		InitGP(array('creditlog'),'P',2);
		...
		if (is_array($creditpay['name']) && !empty($creditpay['name'])) {
			...
			foreach ($creditpay['name'] as $key => $value) {
				if (isset($credit->cType[$value]) && !isset($cpay[$value])) {
					$cpay[$value] = array(
						'rmbrate'	=> intval($creditpay['rmbrate'][$key]),
