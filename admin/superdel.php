...
if ($admintype == 'article') {
	...
	if (empty($action)) {
		...
		InitGP(array('tstart','tend','hits','replies','tcounts','counts','page'),'GP',2);
		if (empty($_POST['step'])) {
			...
			$pages=numofpage($count,$page,$numofpage,"$admin_file?adminjob=superdel&admintype=article&action=$action&fid=$fid&ifkeep=$ifkeep&pstarttime=$pstarttime&pendtime=$pendtime&lstarttime=$lstarttime&lendtime=$lendtime&tstart=$tstart&tend=$tend&hits=$hits&replies=$replies&author=".rawurlencode($author)."&keyword=".rawurlencode($keyword)."&userip=$userip&lines=$lines&ttable=$ttable&tcounts=$tcounts&counts=$counts&");
			$start = ($page-1)*$lines;
		...
		InitGP(array('pstart','pend','page'),'GP',2);
		...
		if (empty($_POST['step'])) {
			...
			$pages = numofpage($rt['sum'],$page,ceil($rt['sum']/$nums),"$admin_file?adminjob=superdel&admintype=article&action=$action&fid=$fid&tid=$tid&pstart=$pstart&pend=$pend&author=".rawurlencode($author)."&keyword=".rawurlencode($keyword)."&userip=$userip&tcounts=$tcounts&counts=$counts&nums=$nums&ptable=$ptable&");
