...
InitGP(array('pcid',P,2));
$pcid && $pcvaluetable = 'pw_pcvalue'.$pcid;
...
if (empty($action)){
	...
	$query = $db->query("SELECT name,pcid FROM pw_forums WHERE pcid!=0 AND type<>'category'");
	while ($rt = $db->fetch_array($query)) {
		foreach (explode(',',$rt['pcid']) as $value) {
			if (!$postcatedb[$value]['forum']) {
				$postcatedb[$value]['forum'] = $rt['name'];
			} else {
				$postcatedb[$value]['forum'] .= ','.$rt['name'];
			}
		}
	...
	if ($step != 'search' || !$count) {
		$count = $db->get_value("SELECT COUNT(tid) as count FROM $pcvaluetable");
		$alltiddb = array();
		$query = $db->query("SELECT tid FROM $pcvaluetable");
		while ($rt = $db->fetch_array($query)) {
			$alltiddb[] = $rt['tid'];
		}
		is_array($alltiddb) && $alltids = implode(',',$alltiddb);
	...
	if ($count > 0) {
		...
		$catedb = array();
		$query = $db->query("SELECT pv.tid,t.fid,t.subject,t.author,t.authorid,t.postdate FROM $pcvaluetable pv LEFT JOIN pw_threads t ON pv.tid=t.tid WHERE 1 $sql ORDER BY t.postdate DESC $limit");
		while ($rt = $db->fetch_array($query)) {
	...
	foreach ($ttable_a as $pw_tmsgs => $tids) {
		...
		$query = $db->query("SELECT t.tid,t.fid,t.authorid,t.replies,t.postdate,t.ptable,tm.aid,t.ifupload FROM pw_threads t LEFT JOIN $pw_tmsgs tm ON tm.tid=t.tid WHERE t.tid IN(".pwImplode($tids).")");
		while (@extract($db->fetch_array($query))) {
			$delnum[$authorid]++;
			$ptable_a[$ptable] = 1;
			...
			$db->update("DELETE FROM $pcvaluetable WHERE tid=".pwEscape($tid));
			$db->update("DELETE FROM pw_pcmember WHERE tid=".pwEscape($tid));
	...
	if (!$_POST['step']) {
		...
		foreach ($vieworder as $key => $value) {
			if ($key > 100) {
				$db->update("UPDATE pw_pcfield SET ".pwSqlSingle(array('ifable'=>$ifable[$key]))." WHERE fieldid=".pwEscape($key));
			}
			$db->update("UPDATE pw_pcfield SET ".pwSqlSingle(array( 'vieworder'=>$value,'ifsearch'=>$ifsearch[$key],'ifasearch'=>$ifasearch[$key],'threadshow'=>$threadshow[$key],'ifmust'=>$ifmust[$key],'textsize'=>$textsize[$key]))." WHERE fieldid=".pwEscape($key));
		}
	...
	if (!$_POST['step']){
		...
		$viewrighthtml = $adminrighthtml  = "<table cellspacing='0' cellpadding='0' border='0' width='100%' align='center'><tr>";
		foreach ($groupdb as $key => $value) {
			...
			if ($value['gptype'] == 'system') {
				...
				$adminrighthtml .= "<td><input class='input' type='checkbox' name='adminright[]' value='".$key."' ".$checked.">".$value['grouptitle']."</td>".$htm_tr;
			}
		}
		$adminrighthtml .= "</tr></table>";
		$num = 0;$htm_tr = '';
		foreach ($groupdb as $key => $value) {
...
function getSearchHtml($data) {
	...
	if (in_array($data['type'],array('radio','select'))) {
		...
		$searchhtml .= "<input id=\"calendar_$data[fieldid]\" type=\"text\" class=\"input\" name=\"field[$data[fieldid]]\" value=\"$data[fieldvalue]\" onclick=\"ShowCalendar(this.id,1)\"/><script language=\"JavaScript\" src=\"js/date.js\"></script>";
