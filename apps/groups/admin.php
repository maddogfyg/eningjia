...
if (empty($action)) {
	...
	if (empty($_POST['step'])) {
		...
		InitGP(array('cname','annouce','descrip','admin'),'P');
		...
		if ($cid != $colony['classid'] && isset($o_classdb[$cid])) {
			...
			if ($db_plist) {
				foreach (explode(',',$db_plist) as $key => $value) {
					$pw_posts = GetPtable($value);
					$db->update("UPDATE pw_argument a LEFT JOIN $pw_posts p ON a.tid=p.tid SET p.fid=" . pwEscape($cid) . " WHERE a.cyid=" . pwEscape($cyid));
				}
			}
			//分类的统计数更新
	...
	if ($step == 'list') {
		...
		if ($db_plist) {
			$p_table = "<select name=\"ptable\"><option value=\"0\">post</option>";
			$p_list = explode(',',$db_plist);
			foreach ($p_list as $value) {
				$p_table .= "<option value=\"$value\"" . ($value == $db_ptable ? ' selected' : '') . ">post$value</option>";
			}
			$p_table .= '</select>';
		}
