...
if (empty($a)) {
	...
	if ($u == $winduid && $space != 1) {
		...
		$count = 0;
		$query = $db->query("SELECT cm.ifadmin,cm.addtime,c.id,c.cname,c.cnimg,c.admin,c.createtime,cm2.uid FROM pw_cmembers cm LEFT JOIN pw_colonys c ON cm.colonyid=c.id LEFT JOIN pw_members cm2 ON c.admin=cm2.username WHERE cm.uid=" . pwEscape($u) . " ORDER BY cm.colonyid DESC");
		while ($rt = $db->fetch_array($query)) {
			if ($rt['cnimg']) {
...
//	footer();
	...
	InitGP(array('page'), '', 2);
	$page < 1 && $page = 1;
	$total = $db->get_one("SELECT COUNT(*) AS sum,SUM(cm.username=c.admin) AS creates FROM pw_cmembers cm LEFT JOIN pw_colonys c ON cm.colonyid=c.id WHERE cm.uid=" . pwEscape($winduid));
	list($pages, $limit) = pwLimitPages($total['sum'], $page, "{$basename}a=my&");

	$group = array();
	$query = $db->query("SELECT c.id,c.cname,c.cnimg,c.admin,c.members FROM pw_cmembers cm LEFT JOIN pw_colonys c ON cm.colonyid=c.id WHERE cm.uid=" . pwEscape($winduid) . $limit);
	while ($rt = $db->fetch_array($query)) {
		if ($rt['cnimg']) {
			list($rt['cnimg']) = geturl("cn_img/$rt[cnimg]",'lf');
		} else {
