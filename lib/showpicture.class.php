...
class PW_ShowPicture{
    ...
    function getPictures($pictureId,$albumId){
         //$album = ($albumId) ? $this->_getAlbumByAlbumId($albumId) : $this->_getAlbumByPictureId($pictureId);
        ...
        return array($photo,$pictures,$prePid,$nextPid);
    }
    ...
    function _calculate($pictureId,$pictureIds){
        ...
        if($total<=$this->_perPage){
            return array($pictureIds,$prePid,$nextPid);
        }
        ...
        return array($needPictureIds,$prePid,$nextPid);
    }
