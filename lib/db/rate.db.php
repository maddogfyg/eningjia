...
class PW_RateDB extends BaseDB {
	...
	function getsByUserId($userId, $objectId, $typeId) {
		return $this->_getConnection ()->get_one ( "SELECT * FROM " . $this->_tableName . " WHERE typeid=" . $typeId . " AND objectid=" . $objectId . " and uid=" . $userId );
	}

	function getsByIp($ip, $objectId, $typeId) {
		return $this->_getConnection ()->get_one ( "SELECT * FROM " . $this->_tableName . " WHERE typeid=" . $typeId . " AND objectid=" . $objectId . " and uid=0 and ip='" . $ip . "'" );
	}

	function countByUserId($userId) {
		$today = explode ( ",", date ( "Y,m,d", time () ) );
		$created_at = mktime ( 0, 0, 0, $today [1], $today [2], $today [0] );
		$result = $this->_getConnection ()->get_one ( "SELECT COUNT(*) AS total FROM " . $this->_tableName . " WHERE uid=" . $userId . " AND created_at>" . $created_at . " LIMIT 1" );
		return $result ['total'];
	}
	...
	function countByIp($ip) {
		$today = explode ( ",", date ( "Y,m,d", time () ) );
		$created_at = mktime ( 0, 0, 0, $today [1], $today [2], $today [0] );
		$result = $this->_getConnection ()->get_one ( "SELECT COUNT(*) AS total FROM " . $this->_tableName . " WHERE uid=0 AND created_at>" . $created_at . " AND ip='" . $ip . "'  LIMIT 1" );
		return $result ['total'];
	}
