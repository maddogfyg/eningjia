...
class PW_STopicBlockDB extends BaseDB {
	...
	function replace($fieldsData) {
		$fieldsData	= $this->_checkData($fieldsData);
		if (!$fieldsData) return null;
		$this->_getConnection()->update("REPLACE INTO ".$this->_tableName." SET " . $this->_getUpdateSqlString($fieldsData));
		return $this->_getConnection()->insert_id();
	}

	function delete($block_id) {
		$this->_getConnection()->update("DELETE FROM ".$this->_tableName." WHERE block_id=". intval($block_id) ." LIMIT 1");
	...
	function update($block_id, $updateData) {
		...
		$this->_getConnection()->update("UPDATE ".$this->_tableName." SET " . $this->_getUpdateSqlString($updateData) . " WHERE block_id=". intval($block_id) ." LIMIT 1");
		return $this->_getConnection()->affected_rows();
	}

	function get($block_id) {
		$data = $this->_getConnection()->get_one("SELECT * FROM ".$this->_tableName." WHERE block_id=".intval($block_id));
		if (!$data) return null;
