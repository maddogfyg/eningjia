 ...
 */
    ...
    function _checkRateConfig($fieldData) {
        ...
        if ($fieldData ['title'] == "" || $fieldData ['icon'] == "" || ! in_array ( $fieldData ['typeid'], $this->_getRateType () ) || ! in_array ( $fieldData ['isopen'], array (0, 1 ) )) {
            return FALSE;
        }
    ...
    //@todo 检查对象的有效性 并获取对象作者的ID
        ...
        if (! $this->getRateResultByOptionId ( $optionId, $objectId )) {
            $fieldData = array ('objectid' => $objectId, 'optionid' => $optionId, 'typeid' => $typeId, 'num' => 1 );
            $this->addRateResult ( $fieldData );
        } else {
            $this->updateRateResultByOptionId ( $optionId, $objectId );
        }
        $fieldData = array ('objectid' => $objectId, 'optionid' => $optionId, 'typeid' => $typeId, 'uid' => $userId, 'created_at' => time (), 'ip' => $ip );
     ...
     * 返回贴子实例：array("rateThread_1" => "最感动","rateThread_2" => "最高兴")
     * 返回日志实例：array("rateDiary_10" => "最感动","rateDiary_12" => "最高兴")
     * 返回相片实例：array("ratePicture_20" => "最感动","ratePicture_21" => "最高兴")
    ...
    function _getThreadById($tid) {
        ...
        $result ['authorUrl'] = "/u.php?action=show&uid=" . $thread ['authorid']; //authorid
        return $result;
    }
    ...
    function _getDiaryByById($did) {
        ...
        $result ['href'] = "/mode.php?m=o&q=diary&u=" . $diary ['uid'] . "&did=" . $did;
        $result ['author'] = $diary ['username'];
        $result ['authorUrl'] = "/u.php?action=show&uid=" . $diary ['uid']; //1
        return $result;
    }
    ...
    function _getPhotoById($pid) {
        ...
        $result ['href'] = "/mode.php?m=o&q=photos&space=1&u=" . $album ['ownerid'] . "&a=view&pid=" . $pid;
        $result ['author'] = $photo ['uploader'];
        $result ['authorUrl'] = "/u.php?action=show&uid=" . $album ['ownerid'];
        return $result;
    }
    ...
    function _getCreditMap() {
        return array ("money" => "-1", "rvrc" => "-2", "credit" => "-3", "currency" => "-4" );
    }
    ...
    function _getReteConfigFilePath() {
        return $this->_getRootPath () . '/data/bbscache/rate_config.php';
    }
	/***********************************************************************************/
    ...
    function _getRateDB() {
        return L::loadDB ( 'Rate' );
    }

    function _getRateResultDB() {
        return L::loadDB ( 'RateResult' );
    }
}
?>
