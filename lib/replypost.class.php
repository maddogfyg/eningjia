...
class replyPost {
	...
	function setPostFloor($pid){
		$sql = "SELECT max(f.floor) as num FROM pw_postsfloor f WHERE f.tid = $this->tid";
		$rt = $this->db->get_one($sql);
		$pfloor = $rt['num'] ? $rt['num'] + 1 : 1;
		$sql = "INSERT INTO pw_postsfloor SET pid=". pwEscape($pid) .", tid=". pwEscape($this->tid) .", floor=". pwEscape($pfloor)."";
		$this->db->update($sql);
	}
