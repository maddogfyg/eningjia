...
$rt = $db->get_one("SELECT fd.topic,fd.top1,fd.top2,fd.lastpost,fd.aid,fd.aids,fd.aidcache,fd.tpost,fd.topic,fd.article ,a.ifconvert,a.author,a.startdate,a.enddate,a.subject,a.content FROM pw_forumdata fd LEFT JOIN pw_announce a ON fd.aid=a.aid WHERE fd.fid=".pwEscape($fid));
...
if ($foruminfo['forumcate'] == 1) {
	...
	if ($foruminfo['cateid'] && $foruminfo['modelid']) {
		...
		$s_modelid = $modelid ? $modelid : $modelids[0];
		$initSearchHtml = $postTopic->initSearchHtml($s_modelid);
	...
	if ($foruminfo['pcid']) {
		...
		if ($pcid > 0) {
			$fielddb = $postTopic->getFieldData($pcid,'one');
			foreach ($fielddb as $key => $value) {
				if($value['threadshow'] == 1) {
					$threadshowfield[$key] = $value;
				}
		...
		$s_pcid = $pcid ? $pcid : $modelids[0];
		$initSearchHtml = $postTopic->initSearchHtml($s_pcid);
...
if (is_numeric($modelid)) {//选择某个信息分类中的某个模板的条件下
	$searchadd .= " AND modelid=".pwEscape($modelid);
	$urladd .= "&modelid=$modelid";
} elseif (is_numeric($pcid)) {//团购活动
	$searchadd .= " AND special=".pwEscape($pcid+20);
	$urladd .= "&pcid=$pcid";
}
if ($searchadd) {
...
if ($fcache < 2) {
	...
	if ($topicids) {
		$topicvalue_table = 'pw_topicvalue'.$modelid;
		$query = $db->query("SELECT * FROM $topicvalue_table WHERE tid IN (" .pwImplode($topicids). ")");
		while ($rt = $db->fetch_array($query)) {
...
if ($db_threadshowpost == 1 && $groupid != 'guest' && !$foruminfo['modelid'] && !$foruminfo['pcid']) {
	$psot_sta = $titletop1 = '';
	$fastpost = 'fastpost';
	$t_exits  = 0;
