...
if (empty($action)) {
	...
	$query = $db->query("SELECT p.pid,p.postdate,t.tid,t.subject FROM $pw_posts p LEFT JOIN pw_threads t USING(tid) WHERE p.authorid=".pwEscape($userdb['uid'])." AND p.postdate>$timestamp-604800 AND p.fid NOT IN($sqloff) ORDER BY p.pid DESC LIMIT 5");
	while ($rt = $db->fetch_array($query)) {
		$rt['postdate'] = get_date($rt['postdate']);
		$postdb[] = $rt;
	}
	...
	if ($p_list) {
		$p_table = "<tr><td id=\"up_post0\" class=\"fav\"><a href=\"{$nurl}ptable=0\">".getLangInfo('other','posttable')."</a></td></tr>";
		foreach ($p_list as $key => $val) {
			$p_table .= "<tr><td id=\"up_post$val\" class=\"fav\"><a href=\"{$nurl}ptable=$val\">".getLangInfo('other','posttable')."$val</a></td></tr>";
		}
		$nurl .= "ptable=$ptable&";
	}
