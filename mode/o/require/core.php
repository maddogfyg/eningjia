...
//$server_url = 'http://apps.phpwind.com';
/**
 * 获取单个好友的信息
 *
 * @param int $friendid
...
function shareEvea($type,$hash,$id){
	...
	if ($src) {
		return (++$share_js ? '<script id="share_js" src="apps/share/js/share.js"></script>' : '').'<img style="cursor: pointer;" src="'.$src.'" onclick="javascript:showShare(\''.$type.'\',\''.$hash.'\',this,\''.$id.'\')"/>';
	} else {
		return '';
	}
}
...
function getCommentDb($type,$typeids){
	...
	while ($rt = $db->fetch_array($query)) {
		...
		if ($rt['groupid'] == 6 && $db_shield && $groupid != 3) {
			...
			$rt['title'] = $wordsfb->convert($rt['title'], new wdstruct($rt['id'],'comments',$rt['ifwordsfb']));
		}
...
function getCommentDbByTypeid($type,$typeid,$page,$url) {
	global $db,$groupid,$db_shield,$db_perpage;
	if(!checkCommType($type)) Showmsg('undefined_action');
	$wordsfb = wordsfb::getInstance();
...
function getAppleftinfo($u,$type=false) {
	...
	$total = 0;
	foreach (unserialize($db_upgrade) as $key => $val) {
		if ($usercredit[$key] && $val) {
			$total += $usercredit[$key]*$val;
		}
	}
	$totalcredit = $total;
