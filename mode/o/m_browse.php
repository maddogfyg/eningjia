...
function browseDiary($num) {
	...
	$uids = '';
	$query = $db->query("SELECT d.did,d.dtid,d.uid,d.username,d.subject,d.r_num,d.c_num,d.postdate,m.icon,m.groupid FROM pw_diary d LEFT JOIN pw_members m ON d.uid=m.uid WHERE d.privacy!=2 ORDER BY d.did DESC LIMIT $num");
	while ($rt = $db->fetch_array($query)) {
		$rt['postdate'] = get_date($rt['postdate'],'m-d H:s');
...
function browseAlbum($num) {
	...
	$query = $db->query("SELECT c.aid,c.aname,c.photonum,c.ownerid,c.owner,c.lastphoto,c.lastpid,c.lasttime,m.groupid FROM pw_cnalbum c LEFT JOIN pw_members m ON c.ownerid=m.uid WHERE c.atype='0' AND c.private='0' ORDER BY c.aid DESC LIMIT $num");
	while ($rt = $db->fetch_array($query)) {
		...
		if ($rt['groupid'] == 6 && $db_shield && $groupid != 3) {
			...
			$rt['aname'] = '';
			$rt['sub_aname'] = appShield('ban_album');
		}
		if ($rt['lastpid']) {
