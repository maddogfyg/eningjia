...
if (!in_array($action,array('banuser','delatc','shield','remind','commend','check','inspect','pingcp'))) {
...
if ($action == "banuser") {
	...
	if (empty($_POST['step'])) {
		...
		if ($range) {
			$db->update('UPDATE pw_members SET userstatus=userstatus|1 WHERE uid='.pwEscape($userdb['uid']));
		} else {
		...
		if ($ifmsg) {
			$message = array(
				...
				'subject'	=> 'banuser_title',
				'content'	=> 'banuser_content_'.$type,
				'other'		=> array(
	...
	if (is_numeric($pid)) {
		$pw_posts = GetPtable('N',$tid);
		$sqlsel = "t.fid,t.subject,t.content,t.postdate,t.ifshield,t.anonymous,";
		$sqltab = "$pw_posts t LEFT JOIN pw_members m ON t.authorid=m.uid";
	...
	if (empty($_POST['step'])) {
		...
		if ($_POST['ifmsg']) {
			$msg = array(
				'toUser'	=> $readdb['username'],
				'subject'	=> 'shield_title_'.$ifshield,
				'content'	=> 'shield_content_'.$ifshield,
				'other'		=> array(
					'manager'	=> $windid,
					'fid'		=> $fid,
					'tid'		=> $tid,
	...
	if (is_numeric($pid)) {
		$pw_posts = GetPtable('N',$tid);
		$sqlsel = "t.fid,t.subject,t.content,t.postdate,t.remindinfo,t.anonymous,";
		$sqltab = "$pw_posts t LEFT JOIN pw_members m ON t.authorid=m.uid";
		$sqladd = " AND t.pid='$pid'";
	} else {
		$pw_tmsgs = GetTtable($tid);
	...
	if (empty($_POST['step'])) {
		...
		if ($ifmsg) {
			...
			$msg = array(
				'toUser'	=> $threaddb['author'],
				'subject'	=> 'inspect_title',
