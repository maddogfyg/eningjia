...
if (!$windid && !in_array($action,array('login','showface','showsmile','getverify','showcard','asearch','poplogin','pingpage'))) {
	Showmsg('not_login');
}
...
//	$rightset = P_unserialize($rightset);
	...
	if ($do == 'vote') {
		...
		if (GetGP('q') == 'y') {
			$db->update("UPDATE pw_debates SET obvote=obvote+1");
			$debate['obvote']++;
			$standpoint = 1;
		} else {
			$db->update("UPDATE pw_debates SET revote=revote+1");
			$debate['revote']++;
			$standpoint = 2;
		}
		...
		if ($_POST['step']) {
			...
			$db->update("UPDATE pw_debates SET".pwSqlSingle($pwSQL));
	...
	if ($type == 'personal' || $type == 'system') {
		...
		if ($msginfo) {
			...
			$wordsfb = wordsfb::getInstance();
			$msginfo['content'] = $wordsfb->convert($msginfo['content']);
	...
	$closebarstyle = '';
	require_once PrintEot('bottom');echo '-->';ajax_footer();
	...
	InitGP(array('tid','thelast','authorid','pcid'));
	...
	if ($thelast != 1) {
		...
		if (empty($_POST['step'])) {
			...
			PostCheck();
			InitGP(array('nums','phone','mobile','address','extra'));

			if ($fieldvalue['limitnum'] && $fieldvalue['limitnum'] - $membernum < $nums) Showmsg('pcjoin_more');
			$nums = (int)$nums;
			if ($nums <= 0) Showmsg('pcjoin_nums');
	...
	$i = $pcid = 0;
	$query = $db->query("SELECT pcmid,uid,pcid,username,nums,totalcash,phone,mobile,address,extra,ifpay FROM pw_pcmember WHERE tid=".pwEscape($tid)." ORDER BY (uid=".pwEscape($winduid).") DESC,ifpay ASC,pcmid DESC $limit");
	while ($rt = $db->fetch_array($query)) {
		if ($i == 0) {
			$pcid = $rt['pcid'];
		}
	...
	$pcmember = $db->get_one("SELECT uid,username,nums,totalcash,phone,mobile,address,extra,ifpay FROM pw_pcmember WHERE pcmid=".pwEscape($pcmid));
	...
	InitGP(array('pcid','pcmid','tid','jointype','ifpay','payway'));
	...
	$rt = $db->get_one("SELECT nums,phone,mobile,address,extra FROM pw_pcmember WHERE pcmid=".pwEscape($pcmid)." AND uid=".pwEscape($winduid));
	if (empty($_POST['step'])) {

		list($checked_1,$checked_2,$checked_3) = explode(',',$rt['extra']);
		$checked_1 && $checked_1 = 'checked';
		$checked_2 && $checked_2 = 'checked';
		$checked_3 && $checked_3 = 'checked';
		...
		$membernum = $db->get_value("SELECT SUM(nums) FROM pw_pcmember WHERE tid=".pwEscape($tid));
		if ($fieldvalue['limitnum'] && $fieldvalue['limitnum'] + $rt['nums'] - $membernum < $nums) Showmsg('pcjoin_more');;
	...
	InitGP(array('tid','id','fieldname','pctype'));
	if (!$tid || !$id || !$fieldname || !$pctype) {
		echo 'fail';
	}
	if ($pctype == 'topic') {
		$tablename .= 'pw_topicvalue'.$id;
	} elseif ($pctype == 'postcate') {
		$tablename .= 'pw_pcvalue'.$id;
	}
