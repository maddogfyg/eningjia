...
//		$url  = "https://www.tenpay.com/cgi-bin/v1.0/pay_gate.cgi?";
		...
		$url .= "desc=".urlencode(getLangInfo('other','userpay_title'))."&sign=$strSign";
		ObHeader($url);
	}
} elseif ($action == 'list') {
	...
	if (empty($_POST['step'])) {
		...
		*/
		...
		refreshto('userpay.php?action=change','bank_creditsuccess');
	}
}
?>