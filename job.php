...
if ($action == 'sign') {
	...
	InitGP(array('atc_content','pcinfo'),'P');
	...
	$atc_content = $wordsfb->convert($atc_content);
	...
	*/
	...
	$memberdb = array();
	$query = $db->query("SELECT username,mobile,phone,address,nums,ifpay,totalcash FROM pw_pcmember WHERE tid=".pwEscape($tid));
	while ($rt = $db->fetch_array($query)) {
		$memberdb[] = $rt;
	}
