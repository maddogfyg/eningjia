...
if (!$action) {
	...
	if ($type == 'notice') {
		...
		if (empty($_POST['step'])) {
			...
			foreach ($ttable_a as $pw_tmsgs => $value) {
				...
				while ($read = $db->fetch_array($query)) {
					...
					if ($read['aid']) $_tids[] = $read['tid'];
				}
			}
			if ($delids) {
				$db->update("DELETE FROM pw_threads WHERE tid IN($delids) AND fid=".pwEscape($fid)." AND ifcheck='0'");
				# memcache refresh
				$threadList = L::loadClass("threadlist");
                                $threadList->refreshThreadIdsByForumId($fid);
			}
			...
			if ($_tids) {
				...
				$attachdb = $pw_attachs->getByTid($_tids,0);
				require_once(R_P.'require/updateforum.php');
				delete_att($attachdb);
				pwFtpClose($ftp);
			}
		...
		if (empty($_POST['step'])) {
			...
			$update_tids = array();
			$query = $db->query("SELECT fid,tid FROM $pw_posts WHERE fid='$fid' AND pid IN($pids)");
			while ($rt = $db->fetch_array($query)) {
				$update_tids[$rt['tid']] ++;
			}
			...
			!$tids && Showmsg('id_error');
			$pw_posts = GetPtable($ptable);
			$query = $db->query("SELECT tid,pid,fid,aid FROM $pw_posts WHERE pid IN($tids)");
			while ($rt = $db->fetch_array($query)) {
			...
			if ($forums['forumadmin'] != stripslashes(",".$windid.",$forumadmin,")) {
				...
				foreach ($newadmin_a as $aid => $value) {
					...
					if ($value && !in_array($value,$newadmin)) {
						$mb = $db->get_one('SELECT uid,groupid,groups FROM pw_members WHERE username='.pwEscape($value));
						if ($mb) {
							$newadmin[] = $value;
							if ($mb['groupid'] == -1) {
								$db->update("UPDATE pw_members SET groupid='5' WHERE uid=".pwEscape($mb['uid']));
								$pwSQL = pwSqlSingle(array(
									'uid'		=> $mb['uid'],
									'username'	=> $value,
									'groupid'	=> 5,
								...
								$db->update('UPDATE pw_members SET groups='.pwEscape($mb['groups']).' WHERE uid='.pwEscape($mb['uid']));
								$pwSQL = pwSqlSingle(array(
									'uid'		=> $mb['uid'],
									'username'	=> $value,
									'groupid'	=> $mb['groupid'],
				...
				if ($oldfadmin) {
					...
					$oldfadmin = pwImplode($oldfadmin,false);
					...
					while ($rt = $db->fetch_array($query)) {
						if (!in_array($rt['username'],$newadmin) && !in_array($rt['username'],$f_admin)) {
							if ($rt['groupid']=='5') {
								$db->update("UPDATE pw_members SET groupid='-1' WHERE uid=".pwEscape($rt['uid'],false));
								$rt['groupid'] = -1;
							} else {
								$rt['groups'] = str_replace(',5,',',',$rt['groups']);
								$rt['groups']==',' && $rt['groups'] = '';
								$db->update("UPDATE pw_members SET groups=".pwEscape($rt['groups'],false)."WHERE uid=".pwEscape($rt['uid'],false));
							}
